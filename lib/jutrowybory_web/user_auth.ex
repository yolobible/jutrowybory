defmodule JutrowyboryWeb.UserAuth do
  use JutrowyboryWeb, :verified_routes

  import Plug.Conn
  import Phoenix.Controller

  alias Jutrowybory.Accounts
  alias Jutrowybory.Accounts.Scope

  # Make the remember me cookie valid for 14 days. This should match
  # the session validity setting in UserToken.
  @max_cookie_age_in_days 14
  @remember_me_cookie "_jutrowybory_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      JutrowyboryWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def require_sudo_mode(conn, _opts) do
    if Accounts.sudo_mode?(conn.assigns.current_scope.user, -10) do
      ...
      conn
      |> put_flash(:error, "Musisz się ponownie zalogować, aby uzyskać dostęp do tej strony.")
  ...
  defp signed_in_path(_conn), do: ~p"/ankieta"
  ...
  @doc """
  Plug wymagający roli administratora.
  """
  def require_admin_user(conn, _opts) do
    case conn.assigns.current_scope do
      ...
      _ ->
        conn
        |> put_flash(:error, "Brak uprawnień administratora.")
        |> redirect(to: ~p"/")
        |> halt()
    end
  end
  ...
  def on_mount(:require_admin, _params, session, socket) do
    ...
    case socket.assigns.current_scope do
      ...
      _ ->
        socket =
          socket
          |> Phoenix.LiveView.put_flash(:error, "Brak uprawnień administratora.")
