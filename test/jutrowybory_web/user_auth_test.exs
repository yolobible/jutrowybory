defmodule JutrowyboryWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, JutrowyboryWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "require_sudo_mode/2" do
    ...
    test "redirects when authentication is too old", %{conn: conn, user: user} do
      ...
      assert Phoenix.Flash.get(conn.assigns.flash, :error) ==
               "Musisz się ponownie zalogować, aby uzyskać dostęp do tej strony."
    end
  end
